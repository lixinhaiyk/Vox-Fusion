...
if __name__ == '__main__':
    ...
    """
    root_path = '/home/u22s151059/project/data/replica/'
    ...
    if data_type == 'replica':
        poses_gt = np.loadtxt(os.path.join(root_path, f'{scene_id}/traj.txt'))
        poses_est = np.load(os.path.join('./',f'logs/replica/room0/2023-03-11-20-57-41/misc/frame_poses.npy'))

        poses_est = np.matmul(poses_gt[0].reshape(4, 4), poses_est)
        poses_gt = torch.from_numpy(poses_gt)[:-1, [3, 7, 11]]
        poses_est = torch.from_numpy(poses_est.reshape(-1, 16))[:, [3, 7, 11]]
        evaluate(poses_gt, poses_est, plot=os.path.join('.', f'logs/replica/room0/2023-03-11-20-57-41/result.png'))

    # for ScanNet
    if data_type == 'scannet':
        pose_dir = os.path.join(root_path, f'scannet_gt/scene{scene_id}')
        poses_est = np.load(os.path.join(root_path, f'scannet_new/scene{scene_id}.npy'))
        poses_est = torch.from_numpy(poses_est.reshape(-1, 16))
        n_pose = poses_est.shape[0]
        poses_gt = []
        poses_pred = []

        for i in range(n_pose):
            pose_gt = np.loadtxt(os.path.join(pose_dir, f'{i}.txt'))
            if not np.isinf(pose_gt).any():
                poses_gt.append(pose_gt)
                poses_pred.append(poses_est[i])

        poses_gt = np.stack(poses_gt, axis=0)
        poses_gt = torch.from_numpy(poses_gt.reshape(-1, 16))[:, [3, 7, 11]]
        poses_pred = torch.stack(poses_pred, dim=0)[:, [3, 7, 11]]
        evaluate(poses_gt, poses_pred, plot=os.path.join(root_path, f'scannet_new/scene{scene_id}.png'))